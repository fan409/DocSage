...
from .upload_jobs import DELETE_STEPS, CRAWL_STEPS, delete_job_manager, upload_job_manager, crawl_job_manager
from .doc_crawler import DOC_SOURCES, crawl_source
...
def _process_crawl_job(job_id: str, source_key: str, max_pages: int) -> None:
    """后台执行爬取 → 解析 → 入库。"""
    import asyncio
    ...
    try:
        ...
        # 1. 爬取
        crawl_job_manager.update_step(job_id, "crawl", 5, "running", f"正在爬取 {config['name']} 文档")
        ...
        async def _do_crawl():
            ...
            async with httpx.AsyncClient(
                ...
                return await crawl_source(source_key, config, max_pages, client)

        pages_crawled = asyncio.run(_do_crawl())
        if pages_crawled == 0:
            raise ValueError("未能爬取到任何页面")
        crawl_job_manager.complete_step(job_id, "crawl", f"爬取完成：{pages_crawled} 页")

        # 2. 解析所有爬取的 .txt 文件并分块
        failed_step = "parse"
        crawl_dir = DOCS_DIR / source_key
        txt_files = sorted(crawl_dir.glob("*.txt"))
        if not txt_files:
            raise ValueError("爬取目录为空，无文件可处理")

        crawl_job_manager.update_step(job_id, "parse", 5, "running", f"正在解析 {len(txt_files)} 个文件")

        all_docs = []
        for txt_file in txt_files:
            try:
                docs = loader.load_document(str(txt_file), txt_file.name)
                all_docs.extend(docs)
            except Exception:
                continue

        parent_docs = [doc for doc in all_docs if int(doc.get("chunk_level", 0) or 0) in (1, 2)]
        leaf_docs = [doc for doc in all_docs if int(doc.get("chunk_level", 0) or 0) == 3]
        if not leaf_docs:
            raise ValueError("解析完成但未生成可检索叶子分块")
        crawl_job_manager.complete_step(
            job_id, "parse",
            f"解析完成：{len(txt_files)} 个文件，父级分块 {len(parent_docs)} 个，叶子分块 {len(leaf_docs)} 个",
        )

        # 3. 父级分块入库
        failed_step = "parent_store"
        crawl_job_manager.update_step(job_id, "parent_store", 20, "running", "正在写入父级分块")
        parent_chunk_store.upsert_documents(parent_docs)
        crawl_job_manager.complete_step(job_id, "parent_store", f"父级分块已入库：{len(parent_docs)} 个")

        # 4. 向量化入库
        failed_step = "vector_store"
        total_leaf = len(leaf_docs)
        crawl_job_manager.update_step(
        ...
        milvus_writer.write_documents(leaf_docs, progress_callback=_on_progress)
        crawl_job_manager.complete_step(job_id, "vector_store", f"向量化入库完成：{total_leaf} 个叶子分块")
        crawl_job_manager.complete_job(job_id, f"爬取 {config['name']} 完成：{pages_crawled} 页，{total_leaf} 个分块入库")
    except Exception as e:
        crawl_job_manager.fail_job(job_id, failed_step, str(e))


@router.get("/documents/crawl/sources")
async def get_crawl_sources(_: User = Depends(require_admin)):
    """返回可用的文档爬取源列表。"""
    return [
        {"key": key, "name": config["name"], "base_url": config["base_url"]}
        for key, config in DOC_SOURCES.items()
    ]
...
async def start_crawl(
    background_tasks: BackgroundTasks,
    source_key: str = "mybatis",
    max_pages: int = 50,
    _: User = Depends(require_admin),
):
    """启动文档爬取任务（管理员）。"""
    if source_key not in DOC_SOURCES:
        raise HTTPException(status_code=400, detail=f"不支持的文档源：{source_key}，可选：{', '.join(DOC_SOURCES.keys())}")
    max_pages = max(1, min(500, max_pages))

    job = crawl_job_manager.create_job(
        DOC_SOURCES[source_key]["name"],
        steps=CRAWL_STEPS,
        current_step="crawl",
        message="等待爬取",
        completion_step="vector_store",
    )
    crawl_job_manager.update_step(job["job_id"], "crawl", 1, "running", "爬取任务已提交")
    background_tasks.add_task(_process_crawl_job, job["job_id"], source_key, max_pages)
    return {"job_id": job["job_id"], "source": source_key, "max_pages": max_pages, "message": f"正在爬取 {DOC_SOURCES[source_key]['name']} 文档"}


@router.get("/documents/crawl/jobs/{job_id}")
async def get_crawl_job(job_id: str, _: User = Depends(require_admin)):
    """查询爬取任务进度。"""
    job = crawl_job_manager.get_job(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="爬取任务不存在或已过期")
    return job
