...
def list_documents(query: str = "") -> str:
    """List all documents in the knowledge base with their names and chunk counts. Use this when the user asks about what documents are in the knowledge base, how many documents there are, or wants to see the document list. Do NOT use search_knowledge_base for these questions."""
    from milvus_client import MilvusManager

    try:
        mgr = MilvusManager()
        if not mgr.has_collection():
        ...
        for item in rows:
            ...
            if filename not in stats:
                stats[filename] = {"file_type": file_type, "count": 0}
            stats[filename]["count"] += 1

        lines = [f"知识库中共有 {len(stats)} 个文档，{len(rows)} 个向量分块：\n"]
        for fname, info in stats.items():
            lines.append(f"- {fname}（{info['file_type']}，{info['count']} 个分块）")
