...
CLASSIFIER_MODEL = os.getenv("CLASSIFIER_MODEL", "qwen-turbo-latest")
...
def _get_classifier_model():
    ...
    if not API_KEY or not CLASSIFIER_MODEL:
        return None
    if _classifier_model is None:
        _classifier_model = init_chat_model(
...
class QuestionRelevance(BaseModel):
    ...
    relevant: str = Field(description="'yes' if the question is related to the knowledge base, 'no' if not")
...
class RAGState(TypedDict):
    ...
    docs: List[dict]
    route: Optional[str]
    question_relevant: Optional[bool]
...
CLASSIFY_PROMPT = (
    "你是一个问题分类器。判断用户的问题是否需要从知识库中检索文档来回答。\n"
    "知识库包含技术文档（如 Spring Framework、MyBatis、LangChain 等）。\n"
    "如果问题需要查询知识库中的文档内容才能回答，输出 relevant=yes。\n"
    "如果问题是闲聊、问候、天气、与文档无关的通用问题，输出 relevant=no。\n"
    "用户问题：{question}"
)


def classify_question_node(state: RAGState) -> RAGState:
    question = state["question"]
    emit_rag_step("🔎", "正在判断问题类型...")

    classifier = _get_classifier_model()
    if not classifier:
        return {"question_relevant": True}

    prompt = CLASSIFY_PROMPT.format(question=question)
    try:
        response = classifier.with_structured_output(QuestionRelevance).invoke(
            [{"role": "user", "content": prompt}]
        )
        is_relevant = (response.relevant or "").strip().lower() == "yes"
    ...
    if is_relevant:
        emit_rag_step("✅", "问题与知识库相关", f"判断: {reason}")
    else:
        emit_rag_step("⏭️", "问题与知识库无关，跳过检索", f"判断: {reason}")
    ...
    return {"question_relevant": is_relevant, "rag_trace": rag_trace}
...
def build_rag_graph():
    ...
    graph.add_conditional_edges(
        "classify_question",
        lambda state: "retrieve_initial" if state.get("question_relevant") else "skip_retrieval",
        {
